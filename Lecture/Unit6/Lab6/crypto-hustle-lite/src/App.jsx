...
function App() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      console.log(Object.entries(data.Data).slice(0, 20))
      setList(data.Data)
    }
  ...
  const searchItems = (inputString) => {
    console.log(Object.entries(list).slice(0, 20))
      ...
      <div className="whole-page">
        ...
        <ul>
          {searchInput.length > 0 ? (
            filterResults.slice(0, 20).map(([key, value]) => (
              <CoinInfo
                key={key}
            ...
            list && Object.entries(list).slice(0, 20).map(([key, value]) => (
        ...
        {/* <ul>
          {Object.entries(list).slice(0, 20).map(([key, value]) => (
              value.PlatformType=="blockchain"?(
                <CoinInfo
                  key={key}
                  image={value.ImageUrl}
